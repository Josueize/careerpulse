...
function Dashboard({ setTab, deadlineJobs=[] }) {
  const jobs = deadlineJobs; const total = jobs.length; const applied = jobs.filter(j=>j.status==="Applied").length; const interviews = jobs.filter(j=>j.status==="Interview").length; const offers = jobs.filter(j=>j.status==="Offer").length; const rejected = jobs.filter(j=>j.status==="Rejected").length; const offerRate = total>0?Math.round((offers/total)*100):0; const interviewRate = total>0?Math.round(((interviews+offers)/total)*100):0; const stats = [{label:"Applications",value:total||0,suffix:"",color:C.accent},{label:"Interviews",value:interviews||0,suffix:"",color:C.accent2},{label:"Offers",value:offers||0,suffix:"",color:C.accent3},{label:"Offer Rate",value:offerRate||0,suffix:"%",color:C.warn}];
